...
namespace AdbTools
{
    public partial class frmMain : Form
    {
        ...
        /// <param name="type">消息类型 0:连接 1:安装apk</param>
        ...
        private void lblApkFile_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(fileName))
            {
                ...
                var fileDate = file.LastWriteTime.ToString("yyyy-MM-dd");
                var fileSize = GetShowSize(fileLength);
                lblFileDate.Visible = true;
                lblFileLength.Visible = true;
                lblFileLength.Text = "大小:" + fileSize;
                lblFileDate.Text = "日期:" + fileDate;
        ...
        private async void btnInstall_Click(object sender, EventArgs e)
        {
            ...
            var result = await Task.WhenAny(tk, Task.Delay(timeout));
            if (result != tk)
            {
                lblApkTips.Text = "安装超时.";
        ...
        private void InstallApk()
        {
            ...
            {
                if (errMsg.Contains("devicenotfound"))
                {
                    UiShowMsg("连接已断开", 1);
                    Invoke(new Action(() =>
                    {
        ...
        /// <returns></returns>
        private string GetShowSize(long len)
        {
            var kb = len / 1024f;
            if (kb >= 1024)
            {
                ...
                {
                    var gb = mb / 1024f;
                    return gb.ToString("f1") + "G";
                }
                else
                {
                    return mb.ToString("f1") + "M";
                }
            }
            else
            {
